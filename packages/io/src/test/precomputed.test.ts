...
const haveDemo = existsSync(join(DEMO, 'info'));

describe('precomputed translator output', () => {
  it('info validates: multiscale image, uint8, raw, one scale', () => {
    if (!haveDemo) { console.log('skip: run npm run gen:precomputed first'); return; }
  ...
  it('chunk files byte-agree with our tiled lane (Fortran [x,y,z,ch])', async () => {
    if (!haveDemo) { console.log('skip: run npm run gen:precomputed first'); return; }
  ...
  it('chunk addressing follows begin-end naming (unsharded spec)', () => {
    if (!haveDemo) { console.log('skip: run npm run gen:precomputed first'); return; }
