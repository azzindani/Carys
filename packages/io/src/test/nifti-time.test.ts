...
describe('nifti 4D time', () => {
  it('cardiac 4D parses nt=30 with 3D dims', needs('cardiac_patient021_4d.nii'), () => {
    const buf = Uint8Array.from(readFileSync(sample('cardiac_patient021_4d.nii')!)).buffer as ArrayBuffer;
    const h = readHeader(buf);
    assert.deepEqual(h.dims, [240, 256, 10]);
    assert.equal(h.nt, 30);
    assert.equal(h.dtype, 'int16');
  });
  it('frames tile the file exactly, frame 0 equals readImage', needs('cardiac_patient021_4d.nii'), () => {
    const raw = Uint8Array.from(readFileSync(sample('cardiac_patient021_4d.nii')!));
  ...
  it('3D files report nt=1, out-of-range frames throw RangeError', needs('cardiac_patient021_4d.nii', 'brain_tumor_BraTS19_CBICA_AQN_1_flair.nii'), () => {
    const buf = Uint8Array.from(readFileSync(sample('brain_tumor_BraTS19_CBICA_AQN_1_flair.nii')!)).buffer as ArrayBuffer;
    const h = readHeader(buf);
    assert.equal(h.nt, 1);
    assert.throws(() => readFrame(h, buf, 1), RangeError);
    const h4 = readHeader(Uint8Array.from(readFileSync(sample('cardiac_patient021_4d.nii')!)).buffer as ArrayBuffer);
